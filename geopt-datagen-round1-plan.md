...
- Status: GREEN.
- Files: physicsnemo/experimental/pnm_pretraining/data/transforms.py
  (WalkSampler now emits supervise_step{0..n_steps-1} instead of one
  flat supervise), examples/cfd/external_aerodynamics/unified_external_aero_recipe/
  datasets/geopt_pretrain.yaml (targets block lists n_steps vector
  fields), conf/model/transolver_pretrain.yaml (out_dim derives from
  n_steps; comment updated), test/experimental/pnm_pretraining/data/test_walk_sampler.py
  (per-step assertions), examples/.../tests/test_pretraining_smoke.py
  (full forward + loss + per-step split now exercised end-to-end).
- Resolves the shape-mismatch flagged by subagent F: the dataset
  declared supervise: vector (3 channels) but the model emitted
  n_steps * 3 (9 for n_steps=3). The recipe's existing per-field
  splitting machinery (output_normalize.split_concat_by_target)
  consumes per-step vectors natively; no recipe-side edits needed.
- Test suite: all green — test_walk_sampler.py: 5 passed;
  test_transolver_pretrain.py: 7 passed; test_pretraining_smoke.py
  (PNM_PR2_E2E_SMOKE=1): 1 passed.
- Commit SHA: see `git log --oneline` for the canonical hash; this
  entry is finalized at commit time.
